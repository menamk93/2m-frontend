...
function Station(props) {
  ...
  const getManager = async(id) => {
    await api.post(`getCurrentCompanyUsersByRole/`,
    {
      role: 'admin_1'
    },
    {
  ...
  function handleDelete() {
    ...
    api.delete(`adminTwo/station/${id}`, 
    {
      headers: authHeader(), 
    })
    .then(res => {
  ...
  const getData =  () => {
    var response =    api.get(`adminTwo/station/${id}`,{
      headers: authHeader(), 
      }).then(res => {
  ...
  const getUsersData = (data) => {
     ...
     api.get(`adminTwo/user/${data.manager_id}`,{
      headers: authHeader(), 
    }).then( res => {
      handleManagerChange(res.data.data)
    })
    var user_tech =  api.get(`adminTwo/user/${data.technician_id}`,{
      headers: authHeader(), 
    }).then(res => {
