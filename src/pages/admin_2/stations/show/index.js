...
function Station (props){
  ...
  const getStation = () => {
    api.get(`adminTwo/station/${id}`,{
      headers: authHeader(), 
    }).then(res => { 
      setStation(res.data.data)
    }).catch(error => console.log(error.response, props))
  };

  const getSensors = () => {
    api.get(`adminTwo/sensor/${id}`,{
      headers: authHeader(), 
    }).then(res => { 
  ...
  const getData = () => {
    ...
    api.get(`adminTwo/sensor/showByDate/${sensor}`,{
      headers: authHeader(), 
    }).then(res => {
  ...
  const getAlerts = () => {
    api.get(`/adminTwo/alert/getAlertFromSensor/${sensor}`,{
      headers: authHeader(), 
    }).then(res => {
          ...
          <Grid container spacing={2} className={classes.body} id='comp'> 
            <Grid container item spacing={2} md={12} direction="row">
              ...
              <Card 
                ...
                body={ <Alert alert={alert} buttonStyle={classes.button}  name={sensorName} stationName={station.name} stationId={station.id} /> }
                bodyHeight={350}
                gridSize={3}
              />          
            </Grid>
